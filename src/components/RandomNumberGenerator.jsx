...
const WidgetHeader = ({ title, color, isGenerating, generateRandom }) => {
  return (
    ...
    <div className="p-4 pb-40 max-w-6xl mx-auto min-h-screen relative">
      ...
      >
        <div className="space-y-6">
          ...
          <p className="text-[8px] text-text-muted font-bold uppercase opacity-50 px-1 text-center">
            Adjust range parameters for the Entropy Engine
          </p>
        </div>
      </HUDContainer>
      ...
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <div className="flex items-center gap-4">
            <p className="text-text-muted font-bold text-xs uppercase tracking-widest opacity-50">
        ...
        {result !== null && (
          <button
            onClick={generateRandom}
            disabled={isGenerating}
            className={`switch-btn switch-btn-blue min-w-50 flex items-center justify-center gap-3 group transition-all hidden md:flex ${isGenerating ? "scale-95 opacity-80" : "hover:scale-105 active:scale-95"}`}
          >
            <RefreshCw
              size={18}
              className={`${isGenerating ? "animate-spin" : "group-hover:rotate-180 transition-transform duration-500"}`}
            />
      ...
      {result === null ? (
        <div className="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in duration-700">
          <div className="material-card p-12 flex flex-col items-center text-center max-w-md w-full border-dashed">
            ...
            <h2 className="text-xl font-black text-text-main uppercase tracking-tighter mb-2">
              Entropy Engine Offline
            </h2>
            <p className="text-xs text-text-muted font-bold uppercase tracking-widest mb-10 opacity-60">
              System requires manual initialization
            </p>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch animate-in fade-in slide-in-from-bottom-8 duration-700">
            {/* Result Widget */}
            <div className="material-card p-0! overflow-hidden flex flex-col min-h-100">
              <WidgetHeader
                title="Output Module"
                color="text-nintendo-blue"
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center relative overflow-hidden group">
                <div
                  ...
                  style={{
                    backgroundImage:
                      "radial-gradient(#00A0E4 1px, transparent 1px)",
